...
// colours:
// - purple: waiting for first touch after boot or didn't receive response from pi -> touch to retry
...
// ---------- UART to Raspberry Pi and Touch Sensor ----------
//ks: mixing const int and define to keep changes minimal
#define RXD2            16  // ESP32 RX2 (connect to Pi TX (GPIO14/BCM14))
const int signalPiPin = 17; // the replacment for the confusing, original ledPin
//#define TXD2 17   // ESP32 TX2 (connect to Pi RX (GPIO15/BCM15)) //ks: NOT USED, if used, change begin in setup()
HardwareSerial PiSerial(2); // Serial2 (allows parallel serial over USB and UART)
...
// ks: moved these up
//const int touchPin = 27;    // Touch input (ESP32: e.g. GPIO27)
//const int ledPin   = 25;    // LED pin (GPIO25) //ks: ledPin?! this goes to Pi -> this is the "button"...
...
void exitErrorMode() {
  Serial.println("✓ Pi communication restored");
}
...
void setup() {
  ...
  PiSerial.begin(115200, SERIAL_8N1, RXD2, -1); //ks: TX not used
  PiSerial.setTimeout(5);
  PiSerial.println("ESP32 READY");

  pinMode(signalPiPin, OUTPUT);
  digitalWrite(signalPiPin, LOW);
